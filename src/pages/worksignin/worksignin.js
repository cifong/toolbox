...
function WorkTimeInfo(props) {
    const { workingHours, workingMinutes, lunchBreakHours, lunchBreakMinutes, punchInTime } = props.data;
    if (!punchInTime) {
        return <></>;
    }
    const totalWorkMinutes = workingHours * 60 + workingMinutes * 1;
    ...
    const getPunchOutTime = () => {
        ...
        if (punchInTime < lunchBreakStart) {
            punchoutTime.setMinutes(punchoutTime.getMinutes() + totalWorkMinutes + totalLunchBreakMinutes);
            return punchoutTime;
        }
